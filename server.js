...
// Authentication for schedule report
app.get('/schedule-report.html', (req, res) => {
  const auth = req.headers.authorization;
  const sessionCookie = req.cookies.adminSession;
  ...
  if (sessionCookie) {
    try {
      ...
      if (username === ADMIN_USERNAME && password === ADMIN_PASSWORD) {
        return res.sendFile('public/schedule-report.html', { root: __dirname });
      }
    } catch (e) {
      // Invalid cookie
    }
  ...
  if (!auth) {
    ...
    return res.send(`
      ...
      <html>
      <head>
        <title>Schedule Report Login</title>
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          body {
            font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
      ...
      <body>
        <div class="login-container">
          ...
          <p>API Data Comparison and Validation</p>
          <form id="loginForm">
            <div class="form-group">
              <label for="username">Username</label>
              <input type="text" id="username" name="username" required autofocus>
        ...
        <script>
          document.getElementById('loginForm').addEventListener('submit', async (e) => {
            ...
            try {
              ...
              if (data.success) {
                localStorage.setItem('adminToken', data.token);
                window.location.href = '/schedule-report.html?token=' + encodeURIComponent(data.token);
              } else {
                messageDiv.textContent = data.message;
                messageDiv.className = 'message error';
                messageDiv.style.display = 'block';
  ...
  // Invalid or missing credentials
  res.status(401).set('WWW-Authenticate', 'Basic realm="Schedule Report"').send('Unauthorized');
});
