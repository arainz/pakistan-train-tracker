...
// Helper function to parse time to minutes (supports both 12-hour and 24-hour formats)
...
// Calculate delay from NextStationETA compared to scheduled time
function calculateDelayFromETA(train, scheduledTime) {
    if (!train.NextStationETA || train.NextStationETA === '--:--' || !scheduledTime) {
        return null;
    }

    try {
        const etaMinutes = parseTimeToMinutes(train.NextStationETA);
...
// Calculate train ETA (prioritize NextStationETA from API)
function calculateTrainETA(train) {
    // Prioritize NextStationETA from API as it's the most accurate real-time data
    if (train.NextStationETA && train.NextStationETA !== '--:--') {
        return formatTime(train.NextStationETA);
    }
    return 'N/A';
}
...
console.log('🚂 CACHE BUSTER - Script version: 1729536020');
...
async function fetchTrainData() {
    ...
    try {
        ...
        if (train.TrainNumber) {
            ...
            const schedResponse = await fetch(`/api/train/${train.TrainNumber}`);
...
function updateTrainInfo(train) {
    ...
    try {
        ...
        // Get scheduled arrival time from train schedule
        ...
        if (scheduleData && scheduleData.length > 0 && train.NextStation) {
            ...
            if (nextStationData) {
                scheduledArrivalTime = nextStationData.ArrivalTime || nextStationData.DepartureTime;
            }
        }

        // Calculate accurate delay from NextStationETA vs scheduled time
        const calculatedDelay = calculateDelayFromETA(train, scheduledArrivalTime);
        const lateBy = calculatedDelay !== null ? calculatedDelay : (train.LateBy || 0);
        ...
        console.log('Calculated delay:', calculatedDelay, 'Late by:', lateBy);

        // Determine movement status
        const movementStatus = speed > 0 ? 'Moving' : 'Stopped';
        const movementClass = speed > 0 ? 'status-moving' : 'status-stopped';

        // Calculate ETA using NextStationETA from API
        const eta = calculateTrainETA(train);
        ...
        document.getElementById('nextETA').textContent = eta;
...
function displayRoute(stations) {
    ...
    stations.forEach((station, index) => {
        ...
        // Get scheduled time for this station
        // For first station (origin), use departure time; for all others, use arrival time
        const isFirstStation = index === 0;
        const scheduledTime = isFirstStation ?
            (station.DepartureTime || station.ArrivalTime || '--:--') :
            (station.ArrivalTime || station.DepartureTime || '--:--');

        // Calculate delay for this station
        let stationDelay = 0;
        if (statusClass === 'current' && trainData) {
            // For current/next station, calculate accurate delay from NextStationETA
            const calculatedDelay = calculateDelayFromETA(trainData, scheduledTime);
            stationDelay = calculatedDelay !== null ? calculatedDelay : (trainData.LateBy || 0);
        } else if (statusClass === 'upcoming' && trainData) {
            // For future stations, use the delay calculated from the current station
            // This provides more accurate ETA than using the old LateBy value
            const currentStationData = scheduleData?.find(s => String(s.StationId) === String(trainData.NextStationId));
            if (currentStationData) {
                // Use arrival time for current station (unless it's the first station)
                const currentStationIndex = scheduleData.findIndex(s => String(s.StationId) === String(trainData.NextStationId));
                const isCurrentFirst = currentStationIndex === 0;
                const currentScheduledTime = isCurrentFirst ?
                    (currentStationData.DepartureTime || currentStationData.ArrivalTime) :
                    (currentStationData.ArrivalTime || currentStationData.DepartureTime);
                const calculatedDelay = calculateDelayFromETA(trainData, currentScheduledTime);
                stationDelay = calculatedDelay !== null ? calculatedDelay : (trainData.LateBy || 0);
            } else {
                stationDelay = trainData?.LateBy || 0;
            }
        } else {
            // For passed stations, use the train's overall delay
            stationDelay = trainData?.LateBy || 0;
        }
        ...
        if (statusClass === 'passed') {
            ...
            displayTime = adjustTime(scheduledTime, stationDelay);
        } else if (statusClass === 'current') {
            // For current station, use NextStationETA if available
            if (trainData && trainData.NextStationETA && trainData.NextStationETA !== '--:--') {
                displayTime = formatTime(trainData.NextStationETA);
            } else {
                displayTime = adjustTime(scheduledTime, stationDelay);
            }
        } else {
        ...
        html += `
            <div class="station-item ${statusClass}">
                ...
                <div class="station-status">
                    <div class="eta-info">
                        ${statusClass === 'passed' ? '' :
                          statusClass === 'current' ? `ETA: ${displayTime}` :
                          displayTime}
                    </div>
                    <div class="actual-info">
                        ${statusClass === 'passed' ? 'Passed' :
                          statusClass === 'current' ?
                            (stationDelay > 0 ? `<span style="color: #e74c3c; font-weight: 600;">Late: ${formatMinutes(stationDelay)}</span>` :
                             stationDelay < 0 ? `<span style="color: #48bb78; font-weight: 600;">Early: ${formatMinutes(Math.abs(stationDelay))}</span>` :
                             '<span style="color: #48bb78; font-weight: 600;">On time</span>') :
                          stationDelay > 0 ? `+${formatMinutes(stationDelay)} delay` :
                          stationDelay < 0 ? `-${formatMinutes(Math.abs(stationDelay))} early` : ''}
                    </div>
                </div>
            </div>
        `;
