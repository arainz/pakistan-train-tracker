...
function displayRoute(stations) {
    ...
    stations.forEach((station, index) => {
        ...
        if (statusClass === 'passed') {
            ...
            const scheduledTimeFormatted = formatTime(scheduledTime) || '--:--';
            const delayText = stationDelay > 0 ? `<span style="color: #e74c3c; font-weight: 600;">Late: ${formatMinutes(stationDelay)}</span>` :
            ...
            timeDisplay = `
                <div style="font-size: 12px; line-height: 1.3;">
                    <div><strong>Scheduled:</strong> ${scheduledTimeFormatted}</div>
                    <div><strong>ETA:</strong> ${displayTime}</div>
                    <div style="margin-top: 4px;">${delayText}</div>
                </div>
            `;
            currentTimeDisplay = `
