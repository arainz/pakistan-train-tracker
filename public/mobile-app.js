...
class MobileApp {
            ...
            html += `
                <div class="train-card" onclick="mobileApp.openTrainDetails('${trainId}')">
                    <div class="train-card-header ${delay > 0 ? 'header-delayed' : 'header-ontime'}">
                        ...
                        <button class="favorite-btn ${this.favoriteTrains.includes(trainId) ? 'favorited' : ''}" onclick="event.stopPropagation(); mobileApp.toggleFavorite('${trainId}')" title="${this.favoriteTrains.includes(trainId) ? 'Remove from favorites' : 'Add to favorites'}">
                            <span>${this.favoriteTrains.includes(trainId) ? '⭐' : '☆'}</span>
                        </button>
                    </div>
    ...
    populateFilteredLiveTrains(filteredTrains, query) {
        ...
        filteredTrains.forEach(train => {
            const trainId = train.TrainID || train.trainId || train.TrainNumber || 'unknown';
    ...
    async updateRouteStationsProgress(train) {
        ...
        if (stations.length === 0) {
            ...
            try {
                const response = await fetch(`/api/train/${train.TrainNumber}`);
                const data = await response.json();
    ...
    populateRouteStations(stations, train) {
        ...
        const currentLocation = train.NextStation || train.Location || '';
        ...
        // Update progress bar with current position (reuse currentLocation from above)
        ...
        if (progressStationIndex === -1) {
            progressStationIndex = Math.floor(stations.length * 0.3); // Estimate current position
        }
    ...
    updateProgressBar(stations, currentStationIndex, train) {
        ...
        if (currentStationIndex >= 0 && stations.length > 1 && totalDistance > 0) {
            ...
            // Base progress on actual distance covered
            const baseProgress = (currentDistance / totalDistance) * 100;

            // Add partial progress within current segment if we have speed data
            const speed = train && train.Speed ? train.Speed : 0;
            if (speed > 0 && currentStationIndex < stations.length - 1) {
                ...
                // Estimate partial progress based on speed (rough estimation)
                // Higher speed = likely further along in the segment
                const speedFactor = Math.min(speed / 100, 0.5); // Cap at 50% of segment
                const partialDistance = segmentDistance * speedFactor;
        ...
        console.log(`🗺️ Progress: ${progressPercentage.toFixed(1)}% (Station ${currentStationIndex + 1}/${stations.length}, Speed: ${trainSpeed} km/h, Distance: ${stations[currentStationIndex]?.DistanceFromOrigin || 0}/${totalDistance} km)`);
    }
    ...
    loadFavorites() {
        ...
        const recentLiveTrains = [...allAvailableTrains].sort((a, b) => {
            // Use current time as fallback to avoid 1970 epoch
            const timeA = new Date(a.UpdateTime || a.updateTime || Date.now());
            const timeB = new Date(b.UpdateTime || b.updateTime || Date.now());
        ...
        this.favoriteTrains.forEach(trainNumber => {
            ...
            // User requested: show second most recent instead of most recent
            // If we have multiple matches, take the second one; if only one, take it
            const liveTrainData = matchingTrains.length > 1 ? matchingTrains[1] : matchingTrains[0];
                ...
                <div class="route-station-item ${statusClass}">
                    ...
                    <div class="station-main-info">
                        ...
                        <div class="station-details-row">
                            ${!isFirst && arrivalTime !== '--:--' ? `<span class="station-time">Arr: ${arrivalTime}</span>` : ''}
                            ${!isLast && departureTime !== '--:--' ? `<span class="station-time">Dep: ${departureTime}</span>` : ''}
                            ${isFirst && departureTime !== '--:--' ? `<span class="station-time">Dep: ${departureTime}</span>` : ''}
                            ${isLast && arrivalTime !== '--:--' ? `<span class="station-time">Arr: ${arrivalTime}</span>` : ''}
                        </div>
                    </div>
    ...
    populateLiveUpdates(liveTrains) {
        ...
        // Sort all trains by most recent update time and show the best ones
        console.log('📊 Sorting all trains by most recent updates...');
        const sortedTrains = [...liveTrains].sort((a, b) => {
            // Use current time as fallback to avoid 1970 epoch
            const timeA = new Date(a.UpdateTime || a.updateTime || Date.now());
            const timeB = new Date(b.UpdateTime || b.updateTime || Date.now());
        ...
        // Show most recent trains (regardless of date - could be today, yesterday, etc.)
        const todayLiveTrains = sortedTrains.slice(0, 15); // Show top 15 most recent
        
        console.log('📅 Most recent trains selected:', todayLiveTrains.length);
        if (todayLiveTrains.length > 0) {
            const firstTrainTime = todayLiveTrains[0].UpdateTime || todayLiveTrains[0].updateTime;
            if (firstTrainTime) {
                const latestUpdate = new Date(firstTrainTime);
                // Only log if it's a valid date
                if (!isNaN(latestUpdate.getTime()) && latestUpdate.getFullYear() !== 1970) {
                    console.log('🕐 Latest train update time:', latestUpdate.toLocaleString());
                } else {
                    console.log('🕐 Latest train update time: Invalid/missing timestamp');
                }
            } else {
                console.log('🕐 Latest train update time: No timestamp available');
            }
        }

        // Get relevant trains based on user location and favorites
        // Since API has no date info, prioritize favorites first
        const relevantTrains = this.getRelevantTrainsForLiveUpdates(todayLiveTrains, true);
        ...
        // Show up to 3 most relevant trains
        const combinedTrains = relevantTrains.slice(0, 3);
    ...
    getRelevantTrainsForLiveUpdates(liveTrains, prioritizeFavorites = false) {
        ...
        const addUniqueTrains = (trains, label) => {
            ...
            console.log(`${label} added:`, added, '(unique)');
        };
        ...
        console.log('⭐ Favorite trains found:', favoriteTrains.length);
        ...
        // 3. If prioritizing favorites (no date info available) and we have favorites, expand to show more
        if (prioritizeFavorites && relevantTrains.length > 0 && relevantTrains.length < 3) {
            console.log('📊 Expanding favorites with recent location-based trains');
            // Add location-based trains that aren't already included
            if (this.nearestStation) {
                const locationBasedTrains = this.getLocationBasedTrains(liveTrains);
                addUniqueTrains(locationBasedTrains.slice(0, 3 - relevantTrains.length), '🗺️ Additional location-based');
            }
        }
        
        // 4. If still no relevant trains, add most recent trains
        if (relevantTrains.length === 0) {
            console.log('📊 No location/favorites - showing most recent trains');
            // Group trains by number and get second most recent for each (same as favorites logic)
            const trainGroups = {};
            ...
            Object.values(trainGroups).forEach(trains => {
                // Sort this train number's instances by update time
                ...
                // Take second most recent if available, otherwise take the first
                const selectedTrain = sortedGroup.length > 1 ? sortedGroup[1] : sortedGroup[0];
                sortedTrains.push(selectedTrain);
            });
            
            // Sort the selected trains by their update times
            sortedTrains.sort((a, b) => {
                const timeA = new Date(a.UpdateTime || a.updateTime || Date.now());
                const timeB = new Date(b.UpdateTime || b.updateTime || Date.now());
                return timeB - timeA;
            });
            addUniqueTrains(sortedTrains.slice(0, 5), '📊 Recent'); // Show top 5 most recent
        }

        // Sort by priority: favorites first, then by update time
    ...
    getLocationBasedTrains(liveTrains) {
        ...
        // Find trains related to current station and nearby stations
        liveTrains.forEach(train => {
            const currentStation = (train.CurrentStation || train.NextStation || train.LastStation || '').toLowerCase();
            const nextStation = (train.NextStation || '').toLowerCase();
            const trainNumber = String(train.TrainNumber || train.trainNumber);
            
            // Check if train is at/near current station
            const isAtCurrentStation = currentStation.includes(currentStationName) || 
                                     currentStationName.includes(currentStation) ||
                                     nextStation.includes(currentStationName) ||
                                     currentStationName.includes(nextStation);
            
            // Check train direction (UP/DOWN based on train number)
            const isUpTrain = trainNumber.includes('UP') || trainNumber.endsWith('UP') || 
                            (parseInt(trainNumber) % 2 === 0); // Even numbers often UP
            const isDownTrain = trainNumber.includes('DN') || trainNumber.includes('DOWN') || 
                              trainNumber.endsWith('DN') || (parseInt(trainNumber) % 2 === 1); // Odd numbers often DOWN
            
            if (isAtCurrentStation) {
                console.log(`🚂 Found train ${trainNumber} (${isUpTrain ? 'UP' : isDownTrain ? 'DOWN' : 'UNKNOWN'}) near ${currentStationName}`);
    ...
    loadFavorites() {
        ...
        const favorites = this.getFavoriteTrains();
        ...
        if (favorites.length === 0) {
            favoritesContainer.innerHTML = `
                <div class="empty-favorites">
                    ...
                    <div class="empty-text">Tap the star icon on any train to add it to favorites</div>
                </div>
            `;
            return;
        }
        ...
        let html = '';
        favorites.forEach(trainNumber => {
            const train = this.trainData.active?.find(t => t.TrainNumber === trainNumber);
            if (train) {
                html += `
                    <div class="favorite-train-card" onclick="mobileApp.openTrainDetails('${train.InnerKey}')">
                        <div class="train-header">
                            ...
                            <div class="favorite-actions">
                                <button class="unfavorite-btn" onclick="event.stopPropagation(); mobileApp.toggleFavorite('${train.TrainNumber}')">⭐</button>
                            </div>
                        </div>
                    </div>
                `;
    ...
    toggleFavorite(trainNumber) {
        let favorites = this.getFavoriteTrains();
        const index = favorites.indexOf(trainNumber);
        const isFavorited = index === -1;
        
        if (isFavorited) {
            favorites.push(trainNumber);
            this.showToast('Added to favorites');
        } else {
        ...
        // Update the favoriteTrains array in the instance
        this.favoriteTrains = favorites;
        
        localStorage.setItem('favoriteTrains', JSON.stringify(favorites));
        
        // Instantly update all favorite buttons for this train
        const favoriteButtons = document.querySelectorAll(`[onclick*="toggleFavorite('${trainNumber}')"]`);
        favoriteButtons.forEach(button => {
            const span = button.querySelector('span');
            if (span) {
                span.textContent = isFavorited ? '⭐' : '☆';
            } else {
